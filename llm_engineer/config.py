...
from llm_engineer.console import console
...
class Config(BaseSettings, extra="allow"):
    ...
    def check_settings(self) -> "Config":
        ...
        if self.model_provider == "custom" and (
            ...
            console.print(
                "Custom API host not found in environment variables. Please set the custom API settings to use custom provider.",
            )
            raise SystemExit(1)
        ...
        if self.model_provider == "anthropic" and self.anthropic_api_key == "":
            console.print(
                "Anthropic API key not found in environment variables. Please set the Anthropic API key to use Anthropic provider.",
            )
            raise SystemExit(1)

        if self.model_provider == "openai" and self.openai_api_key == "":
            console.print(
                "OpenAI API key not found in environment variables. Please set the OpenAI API key to use Litellm provider.",
            )
            raise SystemExit(1)

        if self.tavily_api_key == "":
            console.print("Tavily API key not found in environment variables. Search functionality will be disabled.")
